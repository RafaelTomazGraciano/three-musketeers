    ...
    public class StructSemanticAnalyzer
    {
        ...
        public string? VisitStructGet(ExprParser.StructGetContext context)
        {
            ...
            else
            {
                ...
                // For StructSymbol, it's always "initialized" when declared
                if (!(baseSymbol is StructSymbol) && !baseSymbol.isInitializated)
                {
                    reportError(line, $"Variable '{id}' is used before being initialized");
                }

                // Get the current type
                if (baseSymbol is StructSymbol structSymbol)
                {
                    // For StructSymbol, the type is the struct name
                    currentType = structSymbol.type;
                }
                else
                {
