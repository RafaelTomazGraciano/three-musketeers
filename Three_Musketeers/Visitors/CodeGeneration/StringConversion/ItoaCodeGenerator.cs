...
namespace Three_Musketeers.Visitors.CodeGeneration.StringConversion
{
    public class ItoaCodeGenerator
    {
        ...
        public string VisitItoaConversion([NotNull] ExprParser.ItoaConversionContext context)
        {
            ...
            getCurrentBody().AppendLine($"  {bufferReg} = alloca [20 x i8], align 1");
            //pointer to buffer
            string bufferPtr = nextRegister();
            getCurrentBody().AppendLine($"  {bufferPtr} = getelementptr inbounds [20 x i8], [20 x i8]* {bufferReg}, i32 0, i32 0");
            //sprintf(buffer, "%d", value)
            string resultReg = nextRegister();
            getCurrentBody().AppendLine($"  {resultReg} = call i32 (i8*, i8*, ...) @sprintf(i8* {bufferPtr}, i8* getelementptr inbounds ([3 x i8], [3 x i8]* @.fmt.d, i32 0, i32 0), i32 {intValue})");
